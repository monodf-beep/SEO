...
/** One token, stored like the other API keys, used by the objectives'
 *  conversation checks on Instagram, Facebook, LinkedIn, TikTok and X. */
export function ApifySection({ initialStatus }: { initialStatus: Status }) {
  ...
  async function handleTest() {
    ...
    try {
      const res = await fetch("/api/user/api-keys/test", {
        ...
        body: JSON.stringify({ provider: "apify", login: "token", password: token }),
      });
      const data = await res.json();
      setTestResult(Boolean(data.success));
      if (!data.success) setError("Jeton refusé par Apify");
    } catch {
      setTestResult(false);
  ...
  return (
    <div className="mt-4 rounded-lg border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-medium text-foreground">Apify</h4>
          <p className="text-xs text-muted-foreground">
            Conversations sur Instagram, Facebook, LinkedIn, TikTok et X pour les objectifs
          </p>
        </div>
        {status.connected ? (
        ...
        <div className="mt-4 space-y-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">
              Jeton d&apos;API (Apify Console, Settings, API &amp; Integrations)
            </label>
            <input
              type="password"
              value={token}
          ...
          <p className="text-[11px] text-muted-foreground">
            Chaque recalcul d&apos;objectif lance un scraper par réseau sur votre compte Apify, une dizaine de
            résultats chacun ; le crédit gratuit mensuel couvre un usage courant.
          </p>
        </div>
      )}
    </div>
